...
const METHODS = ['M-Pesa', 'Bank Transfer', 'PayPal', 'Airtel Money']
...
export default function WithdrawalForm({ userId, points }: { userId: string; points: number }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setIsError(true)
      // Surface the friendly DB trigger message
    ...
    setIsError(false)
    setMsg('Withdrawal request submitted! Admin will review within 24–48 hrs.')
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. 0712345678 / account number"
        />
      </div>
      {msg && (
        <p className={`text-sm ${isError ? 'text-red-400' : 'text-brand-400'}`}>{msg}</p>
      )}
      <button type="submit" disabled={loading || points < 100} className="btn-primary flex items-center gap-2">
        {loading
